...
//        barChart.setDescription(null);
     ...
     * @param mData
     */
    public void setDataToChart(TimeDataManage mData, JSONObject datas) {
        this.mData = mData;
        ...
        if(lineChart.getData() != null &&lineChart.getData().getDataSetCount() > 0
            ...
            JSONArray jsonArray = datas.optJSONArray("data");
            ArrayList<String> list = new ArrayList<>();
            for(int i=0;i<jsonArray.length();i++){
                try {
                    JSONArray array = jsonArray.getJSONArray(i);
                    String string = array.getString(5);
                    list.add(string);
                }catch (Exception ex){
                    ex.printStackTrace();
                }
            }
            setXLabels(mData.getOneDayXLabels(landscape,list));
            setShowLabels(true);
            setMarkerView(mData);
