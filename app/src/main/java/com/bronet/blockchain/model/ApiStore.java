...
import com.bronet.blockchain.model.api.HttpLoggingInterceptor;
import com.bronet.blockchain.model.api.cookie.CookiesManager;
...
public class ApiStore {
    ...
    private static void createProxy() {
        ...
        OkHttpClient.Builder builder = new OkHttpClient().newBuilder()//.connectTimeout(20, TimeUnit.SECONDS).readTimeout(20, TimeUnit.SECONDS).writeTimeout(20, TimeUnit.SECONDS)
...
//                        return chain.proceed(request);
                        ...
                        Const.TOKEN=sharePreData.getString(Const.SP_TOKEN);

                        Request original=chain.request();
                        Request request=original.newBuilder().header("Authorization", Const.TOKEN).header("Language", Constants.langae).method(original.method(),
                                original.body()).build();
                        L.test("Const.TOKEN===============>>>"+Const.TOKEN);
                        return chain.proceed(request);
                    }
                })
