...
public class WeekFragment extends BaseFragment {
    ...
    private void SellOutRecord() {
        ApiStore.createApi(ApiService.class)
                .MyTradeListBuy(bean.getInCoinType(),bean.getOutCoinType(),ConstantUtil.ID, 1)
                ...
                .subscribe(new Observer<MyTradeList>() {
                    ...
                    public void onNext(@NonNull MyTradeList myTradeList) {
                        ...
                        if (myTradeList.getStatus()==0){
                           MyTradeList.ResultBean bean = myTradeList.getResult();
                            List<MyTradeList.ResultBean.ItemsBean> itemsBeans =myTradeList.getResult().getItems();
                            sellOutResult = myTradeList.getResult().getItems();
                            sellOutRecordAdapter.setNewData(itemsBeans);
    ...
    private void BuyRecrod() {
        ApiStore.createApi(ApiService.class)
                .MyTradeListSale(bean.getInCoinType(),bean.getOutCoinType(),ConstantUtil.ID, 1)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
