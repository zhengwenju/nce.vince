...
public class MonthFragment extends BaseFragment {
    ...
    protected int setLayoutId() {
        return R.layout.month_list;
    }

    //卖出记录
    private void SellOutRecord() {
        ApiStore.createApi(ApiService.class)
                .MyTradeListBuy(bean.getInCoinType(),bean.getOutCoinType(),ConstantUtil.ID, 2)
    ...
    private void BuyRecrod() {
        ApiStore.createApi(ApiService.class)
                .MyTradeListSale(bean.getInCoinType(),bean.getOutCoinType(),ConstantUtil.ID, 2)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
