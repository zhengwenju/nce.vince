...
public class HalfYearFragment extends BaseFragment {
    ...
    protected int setLayoutId() {
        return R.layout.halfyear_list;
    }
    //卖出记录
    private void SellOutRecord() {
        ApiStore.createApi(ApiService.class)
                .MyTradeListBuy(bean.getInCoinType(),bean.getOutCoinType(),ConstantUtil.ID, 3)
    ...
    private void BuyRecrod() {
        ApiStore.createApi(ApiService.class)
                .MyTradeListSale(bean.getInCoinType(),bean.getOutCoinType(),ConstantUtil.ID, 3)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
